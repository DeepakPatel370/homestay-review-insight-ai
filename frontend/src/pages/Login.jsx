...
export default function Login() {
  ...
  const handleAuthSubmit = async (e) => {
    ...
    if (isRegister) {
      ...
      setLoading(false)
      if (result.success) {
        // Toggle back to login and pre-fill credentials
        setIsRegister(false)
        setPassword('')
      } else {
  ...
  const handleGoogleLogin = () => {
    setLoading(true)
    // Redirect browser directly to Express Google OAuth endpoint
  ...
  return (
    <div className="relative min-h-[80vh] flex flex-col justify-center items-center px-4 py-12">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-indigo-500/5 dark:bg-indigo-500/10 rounded-full blur-[100px] -z-10"></div>
      
      <div className="w-full max-w-md glass-panel rounded-2xl p-8 border border-slate-200 dark:border-white/10 shadow-2xl transition-all duration-300">
        {/* Header */}
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600 dark:text-gray-400 text-xs font-light">
            {isRegister 
              ? 'Sign up to analyze reviews and generate responses.' 
              : 'Sign in to manage reviews and generate response sheets.'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-[10px] text-slate-400 dark:text-gray-500 uppercase tracking-widest font-semibold">
        ...
        <form onSubmit={handleAuthSubmit} className="space-y-4 text-left">
          {/* Name input (only when registering) */}
          {isRegister && (
            <div className="relative">
              <User className="absolute left-3.5 top-[38px] w-4 h-4 text-slate-400 dark:text-gray-600 z-10" />
              <Input
                label="Full Name"
                type="text"
                required
          ...
          {isRegister ? (
            <>
              ...
              <button
                ...
                className="text-sky-600 dark:text-sky-400 font-semibold hover:underline bg-transparent border-none cursor-pointer"
              >
                Sign In
              </button>
            </>
